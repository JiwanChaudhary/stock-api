...
class StockDetailView(APIView):
    def get(self, request, symbol):
        ...
        predicted_price = forecast.iloc[0] if isinstance(forecast, pd.Series) else forecas
        today_price = data['Close'].iloc[-1]
        arima_signal = "Buy" if float(predicted_price) > float(today_price) else "Sell"
        ...
        response_data = {
            'symbol': symbol,
            'name': symbol,  # Replace with actual name if available
            'latest_price': float(data['Close'].iloc[-1]),  # Convert to float to ensure it's a single number
            'rf_predicted_action': rf_action,
            'xgb_predicted_action': xgb_action,
            'arima_signal': arima_signal,
            'rf_accuracy': float(rf_accuracy),  # Accuracy of Random Forest model
            'xgb_accuracy': float(xgb_accuracy),  # Accuracy of XGBoost model
            'historical_data': historical_data,
        }
